...
- Every input, select, multi-select trigger, date/datetime control, and other per-column filter control must use `width: 100%` and `box-sizing: border-box` within the filter cell. Apply this through the owning active-theme PowerGrid adapter when the behavior is universal for that theme; do not scatter duplicate page-level fixes. The usable control width must visually fill the filter cell after its padding, with no narrow control floating inside a wider column.
- Determine each column width from the maximum practical requirement of its header, its filter control, and representative displayed content—not from arbitrary equal widths or the shortest value. A column must never be narrower than the usable minimum of its own filter type. Short identifiers/statuses may stay compact; medium text may wrap deliberately; long fields such as descriptions may wrap across multiple readable lines and do not need to remain on one line, but must receive a practical minimum width that avoids a tall, word-by-word strip. Use explicit theme-owned `min-width`/width rules where necessary and prefer `table-responsive` horizontal scrolling over compressing filters or long content below a usable size.
...
- For PowerGrid, test live global and per-column filtering, date from/to boundaries, persisted reload state, sort, pagination/last page, empty result, and role-specific scope. At the `1280x768` safe area, inspect representative text/select/date filters and assert or visually prove that each control fills 100% of its filter cell's usable width; verify columns are driven by their own filter/content needs, long text wraps readably, and horizontal scrolling appears instead of squeezed controls when the table is wide.
