...
After choosing the representation, run a bounded composition exercise instead of immediately reproducing a generic CRUD layout. Produce two or three plausible compositions from component families that actually exist in the active-theme atlas: one conservative composition close to the strongest native reference, one task-optimized recomposition, and an optional hybrid only when it removes a specific usability problem. Each candidate must preserve familiar interaction, practical density, accessibility, and responsive behavior. Select a candidate only when its advantage can be stated as a concrete user benefit such as faster scanning, clearer hierarchy, fewer context switches, safer actions, or less repeated input. Novelty, empty-space filling, and visual difference are not benefits. If no alternative has a defensible benefit, use the closest native composition and leave subjective embellishment to later developer-directed tuning.
...
- Reuse the selected reference's real component anatomy: wrapper hierarchy, grid/flex behavior, alignment, spacing utilities, typography classes or tokens, control-height family, icon treatment, and responsive breakpoint behavior. Copying only its color or border while inventing the geometry does not satisfy template fidelity.
