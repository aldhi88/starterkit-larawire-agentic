...
- PHP, Laravel, Livewire, and Pest/PHPUnit use the compatible versions locked by project dependencies. Use the active theme proven in source; never treat another registered theme as its visual base. Scramble documents the optional, disabled-by-default `api.<APP_DOMAIN>` gateway. Session/cache drivers are database. Queue defaults to `sync`, while `database` is supported when hosting provides reliable cron-driven processing.
- `starter:install` creates exactly one company, first App, Superuser role, and Superuser user. `starter:sync` projects App metadata and safely bootstraps credentials only when a new local database is empty. Production uses `starter:deploy`. Never seed production business samples.
