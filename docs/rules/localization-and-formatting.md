...
- Format displayed numbers/currency through `Aldhi88\StarterKit\Support\Starter\StarterNumber`, not scattered `number_format()`. Indonesian formatting uses `1.234` and `1.234,5`; IDR shows locale-appropriate symbol/code and decimals only when present.
