<div>
    @if ($otpRequired)
        <div class="text-center mb-6" data-starter-region="otp-introduction">
            <h3 class="h3 mb-2">Verifikasi email</h3>
            <p class="text-body mb-0">Masukkan kode 6 digit yang dikirim ke <strong>{{ $maskedOtpEmail }}</strong>. Kode berlaku selama 5 menit.</p>
        </div>
        ...
        <form wire:submit="verifyOtp" autocomplete="off" data-starter-region="otp-form">
            ...
            <button class="btn btn-primary d-grid w-100" type="submit" wire:loading.attr="disabled" wire:target="verifyOtp">
        ...
        <div class="d-flex justify-content-between align-items-center gap-3 mt-6">
            <button class="btn btn-text-secondary px-0" type="button" wire:click="cancelOtp" wire:loading.attr="disabled">Ganti akun</button>
            <button class="btn btn-text-primary px-0" type="button" wire:click="resendOtp" wire:loading.attr="disabled" wire:target="resendOtp">
    ...
    @elseif ($authenticatorRequired)
        <div class="text-center mb-6" data-starter-region="authenticator-introduction">
            <h3 class="h3 mb-2">Verifikasi authenticator</h3>
            <p class="text-body mb-0">Buka Google Authenticator atau aplikasi TOTP Anda, lalu masukkan kode yang sedang aktif.</p>
        </div>
        ...
        <form wire:submit="verifyAuthenticator" autocomplete="off" data-starter-region="authenticator-form">
            ...
            <button class="btn btn-primary d-grid w-100" type="submit" wire:loading.attr="disabled" wire:target="verifyAuthenticator">
        ...
        <div class="d-flex justify-content-between align-items-center gap-3 mt-6">
            <button class="btn btn-text-secondary px-0" type="button" wire:click="cancelTwoFactor" wire:loading.attr="disabled">Ganti akun</button>
            <button class="btn btn-text-primary px-0" type="button" wire:click="toggleAuthenticatorRecoveryMode" wire:loading.attr="disabled">
    ...
    @else
        <form wire:submit="authenticate" autocomplete="on" data-starter-region="credentials-form">
            ...
            @if ($humanChallengeImage !== '')
                <div class="mb-4" data-starter-region="human-challenge">
                    <label class="form-label" for="human-challenge">Verifikasi keamanan</label>
                    <div class="input-group flex-nowrap mb-2">
                        <span class="input-group-text starter-human-challenge-visual flex-grow-1 justify-content-center p-1 bg-lighter overflow-hidden">
                            <img src="{{ $humanChallengeImage }}" class="d-block" width="168" height="38" alt="Lima angka keamanan acak">
                        </span>
                        <button class="btn btn-outline-secondary" type="button" wire:click="refreshHumanChallenge" wire:loading.attr="disabled" wire:target="refreshHumanChallenge" aria-label="Tampilkan angka keamanan baru">
                            @include('starter.templates.layouts.icon', ['name' => 'refresh', 'class' => 'icon-md m-0'])
                        </button>
                    </div>
                    ...
                    <div class="form-text" id="human-challenge-help">Setiap angka hanya muncul satu kali. Muat ulang jika sulit dibaca.</div>
