...
<div class="modal dashcode-alert-modal dashcode-alert-modal-{{ $type }} fade {{ $visible ? 'show d-block' : '' }} {{ $modalClass }}" id="{{ $id }}" tabindex="-1" role="dialog" aria-hidden="{{ $visible ? 'false' : 'true' }}" @if ($cancelAction) wire:click.self="{{ $cancelAction }}" @else data-starter-modal-close @endif>
